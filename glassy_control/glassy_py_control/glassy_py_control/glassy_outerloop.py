...
class GlassyChallenge(Node):
    def __init__(self):
        ...
        self.innerloop_setpoint_publisher_ = self.create_publisher(glassy_msgs.InnerLoopReferences, '/glassy/innerloop_refs', 1)
        ...
        self.path_info_subscriber_ = self.create_subscription(glassy_msgs.PathInfo, '/glassy/path_info', self.path_info_subscription_callback, 1)
    ...
    def path_info_subscription_callback(self, msg):
        """
        Receives the path info message and updates the local variables that store the path info.
        """
